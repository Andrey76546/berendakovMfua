## Примеры CI/CD в GitHub Actions


- CI
    - [Первый Pipeline](/content/DevOps/CI_CD/Pipelines/hello.md)
    - [Пайплайн для Python](/content/DevOps/CI_CD/Pipelines/Python.md)
    - [Node.js / TypeScript – REST API с тестами](/content/DevOps/CI_CD/Pipelines/NodeJS.md)
    - [Go](/content/DevOps/CI_CD/Pipelines/Go.md)
    - [Rust - консольная утилита с тестами и линтингом](/content/DevOps/CI_CD/Pipelines/Rust_CI_1.md)
    - [PHP с MySQL – приложение с базой данных](/content/DevOps/CI_CD/Pipelines/PHP.md)
    - [C++](/content/DevOps/CI_CD/Pipelines/cpp.md)
    - [Hello Java!](/content/DevOps/CI_CD/Pipelines/hello_java.md)
    - [Pipeline CI на Rust в GitHub Actions](/content/DevOps/CI_CD/Pipelines/Rust_CI_1.md)
    - [Pipeline CI на Rust в GitHub Actions](/content/DevOps/CI_CD/Pipelines/Rust_CI_2.md)
    - []()
- CD
    - [CI/CD на Rust с публикацией в GHCR](/content/DevOps/CI_CD/Pipelines/CD_Rust_CI_2.md)
    - []()

> Если вы обнаружили ошибку в этом тексте - сообщите пожалуйста автору!
